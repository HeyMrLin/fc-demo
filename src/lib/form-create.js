...
var render = function render(vm, handler) {
    ...
    //console.log("handler.rule",handler.rule);
    this.init();
};
...
render.prototype = {
    ...
    inputProps: function inputProps() {
        ...
        var _handler2 = this.handler,
            ...
            attrs = _handler2.rule.attrs; // FIXME 添加attrs以支持普通的 HTML 特性

        return this.props.props(Object.assign(props, { model: 'cptData.' + field, value: this.vm.cptData[field], elementId: unique })).attrs(attrs ? attrs : {}).ref(refName).key('fip' + unique).on(this.event).on('input', function (value) {
...
var handler = function handler(vm, rule) {
    ...
    rule.event = Object.keys(event).reduce(function (initial, eventName) {
        initial['' + eventName] = event[eventName]; // TODO 这里加了on-影响一些事件的触发比如说visible-change
        return initial;
    }, {});
    ...
    mounted: function mounted(vm) {
        ...
        vm.$nextTick(function () {
            Object.keys(vm.cptData).map(function (field) {
                ...
                // console.log("handler",handler);
                // FIXME 这里由于createTmp创建的实例没有对应的renderFactory，使用的是handler中的renderFactory
...
var nodes = { modal: 'Modal', progress: 'el-progress', button: 'el-button', icon: 'i', span: 'span', slider: 'el-slider', rate: 'el-rate', upload: 'el-upload', cascader: 'el-cascader', colorPicker: 'el-color-picker', timePicker: 'el-time-select', datePicker: 'el-date-picker', 'switch': 'el-switch', option: 'el-option', select: 'el-select', checkbox: 'el-checkbox', checkboxGroup: 'el-checkbox-group', radio: 'el-radio', radioGroup: 'el-radio-group', inputNumber: 'el-input-number', input: 'el-input', formItem: 'el-form-item', form: 'el-form', col: 'el-col', row: 'el-row', tree: 'el-tree' }; // TODO 改成了element ui组件
...
var handler = (0, _handler3.default)({
    ...
    mounted: function mounted() {
        // this.el.fileList = this.parseValue;  // FIXME 这里修改了父组件向子组件传的props，会报错，不知道源码为什么会这么写...
...
var handler = (0, _handler2.default)({
    init: function init() {
        var rule = this.rule;
        if (!rule.props.options) rule.props.options = [];
...
var maker = function () {
    ...
    Object.assign(_m, {
        ...
        createTmp: function createTmp(template, vm) {
            ...
            var col = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : { labelWidth: "110px", span: 24 };
            // TODO 添加title参数，为label数据
            var make = commonMaker(title, field);
            make.rule.type = '__tmp'; //radio
...
var render = function render(_ref) {
    ...
    this.rowGroup = []; // TODO 添加row组
    this.layout = options.layout; // TODO 获取布局结构
};
render.prototype = {
    parse: function parse(vm) {
        ...
        // return this.cvm.form(propsData, this.rowGroup);
    },

    /*******TODO 添加row start********/
    makeRow: function makeRow(vn) {
        var _this2 = this;
        ...
        for (var i = 0; i < this.layout.size; i++) {
            arr[i] = [];
            arr[i].push(vn.splice(0, this.layout.get(i + 1)));
        }
        arr.map(function (item) {
            _this2.rowGroup.push(_this2.cvm.row({ props: _this2.options.row || {} }, [].concat(_toConsumableArray(item))));
        });
    },
    ...
    makeFormItem: function makeFormItem(_ref2, VNodeFn) {
        ...
        var propsData = this.props.props({
            ...
            labelFor: unique,
            rules: rule.validate,
            "label-width": rule.col.labelWidth, // TODO element ui使用的是label-width而不是驼峰命名
            required: rule.props.required
        }).key(unique).get();
        return this.cvm.col({
            props: rule.col, style: {
    ...
    makeResetBtn: function makeResetBtn(unique, span) {
        ...
        return this.cvm.col({ props: { span: span, push: 1 } }, [this.cvm.button({
            key: 'frsbtn' + unique, props: this.vm.resetProps, on: {
                "click": function click() {
                    _this3.fCreateApi.resetFields();
                }
            }
    ...
    makeSubmitBtn: function makeSubmitBtn(unique, span) {
        ...
        return this.cvm.col({ props: { span: span } }, [this.cvm.button({
            key: 'fbtn' + unique, props: this.vm.buttonProps, on: {
                "click": function click() {
                    _this4.fCreateApi.submit();
                }
            }
